...
/**
 * Contains class mod_zoomyt\privacy\provider
 *
 * @package    mod_zoomyt
...
class provider implements core_userlist_provider, metadata_provider, request_plugin_provider {
    ...
    public static function get_metadata(collection $coll): collection {
        ...
        $coll->add_database_table('zoomyt_meeting_participants', [
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT zmp.userid
                  ...
                  JOIN {zoomyt} z ON zmd.zoomid = z.id
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        $sql = "SELECT zmp.id,
            ...
            INNER JOIN {modules} m ON m.id = cm.module AND m.name = :modname
            INNER JOIN {zoomyt} z ON z.id = cm.instance
            INNER JOIN {zoomyt_meeting_details} zmd ON zmd.zoomid = z.id
            INNER JOIN {zoomyt_meeting_participants} zmp ON zmp.detailsid = zmd.id
    ...
    public static function delete_data_for_all_users_in_context(context $context) {
        ...
        if ($cm = get_coursemodule_from_id('zoomyt', $context->instanceid)) {
            $meetingdetails = $DB->get_records('zoomyt_meeting_details', ['zoomid' => $cm->instance]);
            foreach ($meetingdetails as $meetingdetail) {
                $DB->delete_records('zoomyt_meeting_participants', ['detailsid' => $meetingdetail->id]);
            }

            $DB->delete_records('zoomyt_meeting_details', ['zoomid' => $cm->instance]);
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            if ($cm = get_coursemodule_from_id('zoomyt', $context->instanceid)) {
                $meetingdetails = $DB->get_records('zoomyt_meeting_details', ['zoomid' => $cm->instance]);
                foreach ($meetingdetails as $meetingdetail) {
                    $DB->delete_records('zoomyt_meeting_participants', ['detailsid' => $meetingdetail->id, 'userid' => $user->id]);
                }
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $DB->delete_records_select('zoomyt_meeting_participants', "userid $insql AND detailsid IN ($sql)", $params);
