...
class update_meetings extends scheduled_task {
    ...
    public function get_name() {
        return get_string('updatemeetings', 'mod_zoomyt');
    }
    ...
    public function execute() {
        ...
        $zoomstoupdate = $DB->get_records('zoomyt', ['exists_on_zoom' => ZOOM_MEETING_EXISTS]);
        $courseidstoupdate = [];
        ...
        foreach ($zoomstoupdate as $zoom) {
            ...
            $zoomactivityurl = new moodle_url('/mod/zoomyt/view.php', ['n' => $zoom->id]);
            ...
            if ($gotinfo) {
                $changed = false;
                $newzoom = populate_zoomyt_from_response($zoom, $response);
                ...
                if ($changed) {
                    ...
                    $DB->update_record('zoomyt', $newzoom);
                ...
                if (!$zoom->recurring && $changed) {
                    ...
                    foreach ($calendarfields as $field) {
                        if ($zoom->$field != $newzoom->$field) {
                            zoomyt_calendar_item_update($newzoom);
