 ...
 * @covers \mod_zoomyt\invitation
 */
final class mod_zoomyt_invitation_test extends advanced_testcase {
    /**
    ...
    protected function setUp(): void {
        parent::setUp();
        set_config('invitationregexenabled', 1, 'zoomyt');
    }
    ...
    public function test_display_message_when_user_has_all_capabilities(): void {
        ...
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
    ...
    /**
     * Test zoom invitation display message for user with only the mod/zoomyt:viewjoinurl capability.
     */
    public function test_display_message_when_user_has_viewjoinurl_capability(): void {
        ...
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
        assign_capability('mod/zoomyt:viewjoinurl', CAP_ALLOW, $role, context_system::instance()->id);
    ...
    /**
     * Test zoom invitation display message for user with only the mod/zoomyt:viewdialin capability.
     */
    public function test_display_message_when_user_has_viewdialin_capability(): void {
        ...
        $role = $this->getDataGenerator()->create_role();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
        assign_capability('mod/zoomyt:viewdialin', CAP_ALLOW, $role, context_system::instance());
        role_assign($role, $user->id, context_course::instance($course->id));
        $message = (new invitation(
            $this->get_mock_invitation_message_scheduledmeeting()
        ))->get_display_string($zoom->cmid, $user->id);
    ...
    public function test_display_message_when_a_regex_pattern_is_empty(): void {
        ...
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitation_joinurl', '', 'zoomyt');
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $role = $this->getDataGenerator()->create_role();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
        ...
        $PAGE->set_url(new moodle_url('/mod/zoomyt/view.php?id=123'));
        $message = (new invitation(
            $this->get_mock_invitation_message_scheduledmeeting()
        ))->get_display_string($zoom->cmid, $user->id);
    ...
    public function test_display_message_when_a_regex_pattern_is_finds_no_match(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitation_joinurl', '/nomatch/mi', 'zoomyt');
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
        $role = $this->getDataGenerator()->create_role();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
    ...
    public function test_display_message_has_invite_removed_if_setting_enabled(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitationremoveinvite', '1', 'zoomyt');
        $course = $this->getDataGenerator()->create_course();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
        $message = (new invitation(
            $this->get_mock_invitation_message_scheduledmeeting()
    ...
    public function test_display_message_does_not_have_invite_removed_if_setting_disabled(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitationremoveinvite', '0', 'zoomyt');
        $course = $this->getDataGenerator()->create_course();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
        $message = (new invitation(
            $this->get_mock_invitation_message_scheduledmeeting()
    ...
    public function test_display_message_has_icallink_removed_if_setting_enabled(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitationremoveicallink', '1', 'zoomyt');
        $course = $this->getDataGenerator()->create_course();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
    ...
    public function test_display_message_does_not_have_icallink_removed_if_setting_disabled(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        set_config('invitationremoveicallink', '0', 'zoomyt');
        $course = $this->getDataGenerator()->create_course();
        $zoom = $this->getDataGenerator()->create_module('zoomyt', ['course' => $course]);
    ...
    public function test_display_message_when_user_has_no_capabilities_with_regex_disabled(): void {
        set_config('invitationregexenabled', 0, 'zoomyt');
        $this->resetAfterTest();
        $this->setAdminUser();
        $user = $this->getDataGenerator()->create_user();
        $course = $this->getDataGenerator()->create_course();
