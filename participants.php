...
$PAGE->set_url('/mod/zoomyt/participants.php', ['id' => $cm->id, 'uuid' => $uuid, 'export' => $export]);

$strname = $zoom->name;
$strtitle = get_string('participants', 'mod_zoomyt');
...
if ($maskparticipantdata) {
    zoomyt_fatal_error(
        ...
        'mod_zoomyt',
        new moodle_url('/mod/zoomyt/report.php', ['id' => $cm->id])
    );
}
...
if (empty($export) || empty($participants)) {
    ...
    if (empty($participants)) {
        notice(get_string('noparticipants', 'mod_zoomyt'), new moodle_url('/mod/zoomyt/report.php', ['id' => $cm->id]));
        echo $OUTPUT->footer();
        exit();
    }
}
...
if ($export != 'xls') {
    ...
    $exporturl = new moodle_url('/mod/zoomyt/participants.php', [
        'id' => $cm->id,
        'uuid' => $uuid,
        'export' => 'xls',
    ]);
