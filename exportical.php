...
if ($config->showdownloadical == ZOOM_DOWNLOADICAL_DISABLE) {
    $disabledredirecturl = new moodle_url('/mod/zoomyt/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicaldisabled', 'mod_zoomyt', $disabledredirecturl);
}
...
if ($zoom->recurring && $zoom->recurrence_type == ZOOM_RECURRINGTYPE_NOTIME) {
    $errorredirecturl = new moodle_url('/mod/zoomyt/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicalrecurringnofixed', 'mod_zoomyt', $errorredirecturl);
}
...
$descriptiontext = get_string('calendarjoinurl', 'mod_zoomyt', $CFG->wwwroot . '/mod/zoomyt/view.php?id=' . $cm->id);
if (!empty($convertedtext)) {
    $descriptiontext .= get_string('calendardescriptionintro', 'mod_zoomyt', $convertedtext);
}
...
$params = ['modulename' => 'zoomyt', 'instance' => $zoomyt->id];
...
if (empty($events)) {
    ...
    $errorredirecturl = new moodle_url('/mod/zoomyt/view.php', ['id' => $id]);
    throw new moodle_exception('err_downloadicalrecurringempty', 'mod_zoomyt', $errorredirecturl);
}

// Iterate over all events.
// We will add each event as an individual iCal event.
foreach ($events as $event) {
    $icalevent = zoomyt_helper_icalendar_event($event, $descriptiontext);
